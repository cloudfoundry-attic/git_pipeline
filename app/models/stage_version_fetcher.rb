class StageVersionFetcher
  ...
  def fetch
    ...
    git_tags.map.with_index { |git_tag, i|
      stage_from_git_tags(git_tags[i-1], git_tag) if i > 0
    }.compact.reverse
  end

  private

  def stage_from_git_tags(git_tag_from, git_tag_to)
    git_log = GitLog.from_symbolic_range(
      @git_repo, git_tag_from.name, git_tag_to.name)
    Stage.new("Release #{git_tag_from.name}", git_log)
  end
